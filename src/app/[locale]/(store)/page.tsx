...
export default async function StoreHomePage() {
  ...
  const description = settings.data.seoDescription || t("catalogDescription");
  ...
  return (
    <div className="space-y-16 sm:space-y-24">
      ...
      <ScrollReveal><PromotionalBanner badge={t("featuredSpotlight")} title={t("consideredCatalog")} description={t("catalogDescription")} ctaText={t("exploreCollection")} /></ScrollReveal>
      <ScrollReveal><section className="grid items-center gap-8 rounded-[var(--radius-2xl)] border border-[var(--border)] bg-[var(--surface-card)] p-8 shadow-[var(--shadow-card)] sm:p-12 lg:grid-cols-2"><div className="space-y-4"><Badge variant="secondary" size="sm" className="uppercase font-semibold tracking-wider text-[10px]">{t("storeStandard")}</Badge><h2 className="text-2xl font-bold tracking-tight sm:text-3xl">{t("clearReliableCatalog")}</h2><p className="text-sm leading-relaxed text-[var(--text-secondary)]">{t("storeStandardDescription")}</p><div className="grid gap-4 pt-4 sm:grid-cols-2"><div className="flex items-start gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface-muted)]/50 p-3.5"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-[var(--primary)]" /><div><h4 className="text-xs font-bold">{t("verifiedCatalog")}</h4><p className="mt-0.5 text-[11px] text-[var(--text-secondary)]">{t("serverBackedData")}</p></div></div><div className="flex items-start gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface-muted)]/50 p-3.5"><Zap className="mt-0.5 h-5 w-5 shrink-0 text-[var(--primary)]" /><div><h4 className="text-xs font-bold">{t("orderReady")}</h4><p className="mt-0.5 text-[11px] text-[var(--text-secondary)]">{t("availabilityChecked")}</p></div></div></div></div><div className="relative aspect-video overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] lg:aspect-square"><Image src="/placeholders/banner-placeholder.svg" alt={t("clearReliableCatalog")} fill sizes="(max-width: 1024px) 100vw, 32rem" className="object-cover" /></div></section></ScrollReveal>
