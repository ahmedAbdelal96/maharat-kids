...
export default async function CustomerOrderPage({ params }: { params: Promise<{ locale: string; orderNumber: string }> }) {
  ...
  return <OrderDetailsView order={result.data} returnEligibility={eligibility.success ? eligibility.data : null} reviewData={reviews.success ? reviews.data : null} currency={settings.success ? settings.data.currency : result.data.currency} locale={locale === "ar" ? "ar" : "en"} />;
}
