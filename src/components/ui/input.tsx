    ...
    return (
      <div className="w-full">
        <div className="relative flex items-center">
          {icon && (
            <div className="absolute start-3.5 flex items-center pointer-events-none text-[var(--text-muted)]">
              {icon}
            </div>
          )}
          <input
            type={effectiveType}
            className={cn(
              "flex h-11 w-full rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] px-3.5 py-2 text-sm text-[var(--foreground)] placeholder:text-[var(--text-muted)] transition-all",
          ...
          {showToggle && (
            <button
              ...
              className="absolute end-3 flex items-center text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors focus:outline-none cursor-pointer"
            >
              {showPassword ? (
