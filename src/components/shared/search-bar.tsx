...
export function SearchBar({
  ...
  return (
    <form ref={containerRef} role="search" onSubmit={handleSubmit} className={cn("relative flex w-full items-center", className)}>
      <Search aria-hidden="true" className="pointer-events-none absolute start-3.5 h-4 w-4 text-[var(--text-muted)]" />
      <input
        type="search"
        ...
        className="h-10 w-full rounded-full border border-[var(--border)] bg-[var(--surface-muted)]/60 ps-10 pe-10 text-sm text-[var(--foreground)] placeholder:text-[var(--text-muted)] transition-all focus:border-[var(--primary)] focus:bg-[var(--surface)] focus:outline-none focus:ring-2 focus:ring-[var(--ring)]"
      />
      {isLoading && <LoaderCircle aria-label={t("loadingSuggestions")} className="absolute end-3.5 h-4 w-4 animate-spin text-[var(--primary)]" />}
      {!isLoading && query && <button type="button" onClick={() => { setQuery(""); setResult(null); setIsOpen(false); }} className="absolute end-3 p-1 text-[var(--text-muted)] transition-colors hover:text-[var(--text-primary)] focus:outline-none cursor-pointer" aria-label={t("clearSearch")}><X className="h-3.5 w-3.5" /></button>}

      {showSuggestions && (
        <div id="search-suggestions" role="listbox" aria-label={t("searchSuggestions")} className="absolute start-0 top-full z-50 mt-2 max-h-[min(28rem,calc(100vh-8rem))] w-full min-w-[min(20rem,calc(100vw-2rem))] overflow-y-auto rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface-card)] p-2 shadow-[var(--shadow-elevated)]">
          ...
          {!isLoading && !requestError && <button type="button" onClick={() => submitSearch()} className="mt-1 block min-h-11 w-full border-t border-[var(--border-subtle)] px-3 py-3 text-start text-xs font-semibold text-[var(--primary)] hover:bg-[var(--surface-muted)]">{t("viewAllResults", { query: normalizedQuery })}</button>}
        </div>
      )}
    </form>
  );
