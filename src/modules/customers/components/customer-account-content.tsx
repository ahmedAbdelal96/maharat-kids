...
export function CustomerAccountContent({ initialData, currency = "USD", initialSection = "profile" }: { initialData: CustomerAccountData; currency?: string; initialSection?: AccountSection }) {
  ...
  async function savePassword(event: FormEvent<HTMLFormElement>) {
    ...
    if (result.success) { setPasswordForm({ currentPassword: "", newPassword: "", confirmPassword: "" }); setPasswordMessage(t("passwordChanged")); }
    else setErrorMessage(formError(result));
    setIsSaving(false);
  }
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:items-start">
      <aside className="lg:sticky lg:top-24">
        <div className="mb-3 flex items-end justify-between lg:block"><div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[var(--text-muted)]">{t("title")}</p><p className="mt-1 text-sm text-[var(--text-secondary)]">{t("navDescription")}</p></div></div>
        <nav aria-label={t("title")} className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-1 lg:gap-1.5">
          {navigation.map((item) => { const Icon = item.icon; const selected = item.id === activeSection; return <button key={item.id} type="button" aria-current={selected ? "page" : undefined} onClick={() => selectSection(item.id)} className={`flex min-h-16 items-center gap-3 rounded-[var(--radius-md)] border p-3 text-left transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring)] lg:min-h-0 ${selected ? "border-[var(--primary)]/25 bg-[var(--primary-soft)] text-[var(--text-primary)]" : "border-[var(--border)] bg-[var(--surface-card)] text-[var(--text-secondary)] hover:border-[var(--primary)]/40 hover:text-[var(--text-primary)]"}`}><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--radius-sm)] ${selected ? "bg-[var(--primary)] text-[var(--primary-foreground)]" : "bg-[var(--surface-muted)] text-[var(--text-muted)]"}`}><Icon className="h-4 w-4" /></span><span className="min-w-0"><span className="block text-xs font-bold">{t(item.id)}</span><span className="hidden truncate text-[10px] lg:block">{t(`${item.id}Description`)}</span></span>{item.id === "addresses" && addresses.length > 0 && <span className="ml-auto text-[10px] font-bold text-[var(--text-muted)]">{addresses.length}</span>}{item.id === "orders" && initialData.orders && initialData.orders.length > 0 && <span className="ml-auto text-[10px] font-bold text-[var(--text-muted)]">{initialData.orders.length}</span>}</button>; })}
          <Link href="/account/returns" className="flex min-h-16 items-center gap-3 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface-card)] p-3 text-left text-[var(--text-secondary)] transition-all hover:border-[var(--primary)]/40 hover:text-[var(--text-primary)] lg:min-h-0"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--radius-sm)] bg-[var(--surface-muted)] text-[var(--text-muted)]"><RotateCcw className="h-4 w-4" /></span><span className="min-w-0"><span className="block text-xs font-bold">{t("returns")}</span><span className="hidden truncate text-[10px] lg:block">{t("returnsDescription")}</span></span></Link>
        </nav>
      </aside>

      <motion.div key={activeSection} {...contentMotion} className="min-w-0 space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold text-[var(--primary)]">{t(activeSection)}</p><h2 className="mt-1 text-2xl font-extrabold tracking-tight text-[var(--text-primary)]">{t(activeSection === "profile" ? "yourProfile" : activeSection === "addresses" ? "deliveryAddresses" : activeSection === "favorites" ? "savedFavorites" : activeSection === "orders" ? "yourOrders" : activeSection === "reviews" ? "yourReviews" : activeSection === "notifications" ? "notificationsTitle" : "accountSecurity")}</h2><p className="mt-1 text-sm text-[var(--text-secondary)]">{t(`${activeSection}Description`)}</p></div>{activeSection === "profile" && !profileEdit && <Button size="sm" variant="outline" className="gap-1.5 self-start" onClick={() => setProfileEdit(true)}><Pencil className="h-3.5 w-3.5" />{t("editProfile")}</Button>}{activeSection === "addresses" && <Button size="sm" className="gap-1.5 self-start" onClick={openCreateAddress}><Plus className="h-4 w-4" />{t("addAddress")}</Button>}</div>
        ...
        {activeSection === "profile" && <Card><CardHeader><CardTitle className="flex items-center gap-2"><UserRound className="h-4 w-4 text-[var(--primary)]" />{t("personalInformation")}</CardTitle><CardDescription>{profileEdit ? t("updateDetails") : t("savedDetails")}</CardDescription></CardHeader><CardContent>{profileEdit ? <form onSubmit={saveProfile} className="max-w-xl space-y-4"><label className="block text-xs font-semibold">{t("name")}<Input value={profileForm.name} onChange={(event) => setProfileForm((current) => ({ ...current, name: event.target.value }))} placeholder={t("yourName")} /></label><label className="block text-xs font-semibold">{t("email")}<Input type="email" required value={profileForm.email} onChange={(event) => setProfileForm((current) => ({ ...current, email: event.target.value }))} /></label><label className="block text-xs font-semibold">{t("phone")}<Input value={profileForm.phone} onChange={(event) => setProfileForm((current) => ({ ...current, phone: event.target.value }))} placeholder="010xxxxxxxx" /></label><label className="flex items-start gap-2 rounded-[var(--radius-md)] bg-[var(--surface-muted)] p-3 text-xs font-semibold"><input type="checkbox" checked={profileForm.marketingConsent} onChange={(event) => setProfileForm((current) => ({ ...current, marketingConsent: event.target.checked }))} className="mt-0.5 h-4 w-4 accent-[var(--primary)]" /><span>{t("receiveOffers")} <span className="font-normal text-[var(--text-muted)]">({t("optional")})</span></span></label><div className="flex flex-wrap gap-2"><Button type="submit" isLoading={isSaving}>{t("saveChanges")}</Button><Button type="button" variant="outline" disabled={isSaving} onClick={() => { setProfileEdit(false); setProfileForm({ name: profile.name ?? "", email: profile.email, phone: profile.phone ?? "", marketingConsent: profile.marketingConsent }); }}>{t("cancel")}</Button></div></form> : <div className="grid gap-4 sm:grid-cols-3"><div className="rounded-[var(--radius-md)] bg-[var(--surface-muted)] p-4"><p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--text-muted)]">{t("name")}</p><p className="mt-1 text-sm font-bold">{profile.name || t("notProvided")}</p></div><div className="rounded-[var(--radius-md)] bg-[var(--surface-muted)] p-4"><p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--text-muted)]">{t("email")}</p><p className="mt-1 break-all text-sm font-bold">{profile.email}</p></div><div className="rounded-[var(--radius-md)] bg-[var(--surface-muted)] p-4"><p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--text-muted)]">{t("phone")}</p><p className="mt-1 text-sm font-bold">{profile.phone || t("notProvided")}</p></div></div>}</CardContent></Card>}

        {activeSection === "addresses" && <div className="space-y-4">{addresses.length === 0 ? <Card><CardContent className="flex flex-col items-center px-6 py-12 text-center"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-[var(--primary-soft)] text-[var(--primary)]"><MapPin className="h-5 w-5" /></div><h3 className="mt-4 text-base font-bold">{t("firstAddress")}</h3><p className="mt-1 max-w-sm text-sm text-[var(--text-secondary)]">{t("addressHint")}</p><Button className="mt-5 gap-1.5" onClick={openCreateAddress}><Plus className="h-4 w-4" />{t("addAddress")}</Button></CardContent></Card> : <div className="grid gap-4 sm:grid-cols-2">{addresses.map((address) => <Card key={address.id}><CardContent className="space-y-4 p-5"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-2"><MapPin className="h-4 w-4 text-[var(--primary)]" /><h3 className="font-bold">{address.label}</h3></div>{address.isDefault && <Badge variant="secondary" size="sm">{t("default")}</Badge>}</div><div className="text-xs leading-5 text-[var(--text-secondary)]"><p className="font-semibold text-[var(--text-primary)]">{address.recipientName}</p><p>{address.phone}</p><p>{formatCustomerAddress(address)}</p>{address.notes && <p className="mt-1"><span className="font-semibold text-[var(--text-primary)]">{t("notes")}:</span> {address.notes}</p>}</div><div className="flex flex-wrap gap-2 border-t border-[var(--border)] pt-3"><Button variant="ghost" size="sm" onClick={() => openEditAddress(address)}><Pencil className="h-3.5 w-3.5" />{t("editProfile")}</Button>{!address.isDefault && <Button variant="outline" size="sm" disabled={isSaving} onClick={() => makeDefault(address)}>{t("makeDefault")}</Button>}<Button variant="ghost" size="icon" aria-label={`${t("delete")} ${address.label}`} disabled={isSaving || isDeletingAddress} onClick={() => requestDeleteAddress(address)}><Trash2 className="h-4 w-4 text-[var(--destructive)]" /></Button></div></CardContent></Card>)}</div>}</div>}
        ...
        {activeSection === "orders" && <Card><CardContent className="p-4 sm:p-6">{!initialData.orders?.length ? <div className="flex flex-col items-center py-10 text-center"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-[var(--primary-soft)] text-[var(--primary)]"><ShoppingBag className="h-5 w-5" /></div><h3 className="mt-4 text-base font-bold">{t("emptyOrders")}</h3><p className="mt-1 text-sm text-[var(--text-secondary)]">{t("ordersHint")}</p><Link href="/products"><Button className="mt-5">{t("browseProducts")}</Button></Link></div> : <div className="grid gap-3">{initialData.orders.map((order) => <Link key={order.id} href={`/account/orders/${encodeURIComponent(order.orderNumber)}`} className="group rounded-[var(--radius-lg)] border border-[var(--border)] p-4 transition-colors hover:border-[var(--primary)]/50 hover:bg-[var(--surface-muted)]/40"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold text-[var(--text-primary)]">{t("order", { number: order.orderNumber })}</p><p className="mt-1 text-[11px] text-[var(--text-secondary)]">{formatDate(order.createdAt)} · {order.paymentMethodName}</p></div><Badge variant={orderVariant(order.status)} size="sm">{order.status}</Badge></div><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4"><div><p className="text-[10px] font-semibold uppercase tracking-wide text-[var(--text-muted)]">{t("total")}</p><p className="mt-1 text-sm font-extrabold">{formatMoney(order.total, order.currency)}</p></div><div><p className="text-[10px] font-semibold uppercase tracking-wide text-[var(--text-muted)]">{t("payment")}</p><p className="mt-1 text-xs font-semibold">{order.paymentStatus}</p></div><div className="col-span-2 flex items-end justify-end"><span className="text-xs font-bold text-[var(--primary)] transition-transform group-hover:translate-x-0.5">{t("viewOrder")}</span></div></div></Link>)}</div>}</CardContent></Card>}
