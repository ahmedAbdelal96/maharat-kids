...
export function AdminCustomersClient({
  ...
  initialCustomers: AdminCustomerListPage;
  canUpdate: boolean;
  currency: string;
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-[var(--text-primary)]">
            ...
            <span className="text-base font-medium text-[var(--text-muted)]">
              {formatNumber(initialCustomers.total)}
            </span>
          </h1>
          <p className="mt-1 text-xs text-[var(--text-secondary)] sm:text-sm">
            {t("manageCustomerData")}
          </p>
        </div>
      ...
      <form
        className="grid gap-3 rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-card)] p-4 shadow-[var(--shadow-card)] sm:grid-cols-2 lg:grid-cols-5"
        method="get"
      >
        ...
        <select
          ...
          className="h-9 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface-card)] px-3 text-xs text-[var(--foreground)]"
        >
        ...
        <select
          name="purchase"
          defaultValue={query.purchase ?? ""}
          className="h-9 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface-card)] px-3 text-xs text-[var(--foreground)]"
        >
          <option value="">{t("allOrders")}</option>
          <option value="HAS_ORDERS">{t("hasOrders")}</option>
          <option value="NO_ORDERS">{t("noOrders")}</option>
        </select>
        <select
          name="marketing"
          ...
          className="h-9 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface-card)] px-3 text-xs text-[var(--foreground)]"
        >
          <option value="">{t("marketingConsent")}</option>
          <option value="OPTED_IN">{t("optedIn")}</option>
          <option value="NOT_OPTED_IN">{t("notOptedIn")}</option>
        </select>
        <div className="flex items-center gap-2 sm:col-span-2 lg:col-span-5">
          ...
          <select
            ...
            className="h-9 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface-card)] px-3 text-xs text-[var(--foreground)]"
          >
            <option value="">{t("allValue")}</option>
            <option value="HAS_SPENT">{t("hasSpent")}</option>
            <option value="NEVER_PURCHASED">{t("neverPurchased")}</option>
          </select>
          ...
          <Link
            ...
            className="text-xs font-semibold text-[var(--text-secondary)] hover:text-[var(--primary)]"
          >
            {t("clear")}
          </Link>
        </div>
      ...
      {customers.length === 0 ? (
        <Card>
          <CardContent className="py-10 text-center text-sm text-[var(--text-secondary)]">
            <UserRound className="mx-auto mb-3 h-8 w-8 text-[var(--text-muted)]" />
            {t("noCustomers")}
          </CardContent>
        </Card>
      ) : (
              ...
              <TableRow key={customer.id}>
                <TableCell>
                  ...
                  >
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--surface-muted)] text-xs font-bold text-[var(--text-secondary)]">
                      {(customer.name || customer.email || customer.phone || "C")
                        .charAt(0)
                        .toUpperCase()}
                    </div>
                    <div>
                      <span className="text-xs font-semibold text-[var(--text-primary)] group-hover:text-[var(--primary)]">
                ...
                <TableCell className="text-xs text-[var(--text-secondary)]">
                  ...
                  <p className="text-[10px] text-[var(--text-muted)]">
                    {formatNumber(customer.loginCount)} {t("successful")}
                  </p>
                </TableCell>
                ...
                <TableCell>
                  <Badge
                    variant={customer.marketingConsent ? "success" : "outline"}
                    size="sm"
                  >
                    {customer.marketingConsent ? t("optedIn") : t("noResults")}
                  </Badge>
                </TableCell>
                <TableCell>
                  <Badge variant={statusVariant(customer.status)} size="sm">
                    {customer.status}
                  </Badge>
                </TableCell>
                <TableCell>
                    ...
                    {canUpdate &&
                      (customer.status === "SUSPENDED" ? (
                        <Button
                          ...
                          disabled={isSaving}
                          onClick={() =>
                            void requestStatusChange(customer, "ACTIVE")
                          }
                        >
                          {t("activate")}
                        </Button>
                        ...
                        <Button
                          ...
                          disabled={isSaving}
                          onClick={() =>
                            requestStatusChange(customer, "SUSPENDED")
                          }
                        >
                          {t("suspend")}
                        </Button>
      ...
      {initialCustomers.totalPages > 1 && (
        <div className="flex items-center justify-between rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface-card)] px-4 py-3 text-xs">
          <span className="text-[var(--text-secondary)]">
            {t("page", {
              current: initialCustomers.page,
              total: initialCustomers.totalPages,
            })}
          </span>
          <div className="flex gap-2">
            {initialCustomers.page > 1 && (
              <Link href={pageHref(initialCustomers.page - 1, query)}>
                <Button variant="outline" size="sm">
                  {productLabels("previous")}
                </Button>
              </Link>
            )}
            {initialCustomers.page < initialCustomers.totalPages && (
              <Link href={pageHref(initialCustomers.page + 1, query)}>
                <Button variant="outline" size="sm">
                  {productLabels("next")}
                </Button>
      ...
      <Modal
        ...
        onClose={() => {
          if (!isSaving) setSuspendTarget(null);
        }}
        title={t("suspendCustomer")}
        description={
          suspendTarget
            ? t("suspendDescription", {
                name: suspendTarget.name || suspendTarget.email || suspendTarget.phone || "Customer",
              })
            : undefined
        }
        maxWidth="sm"
        ...
        <div className="space-y-5">
          ...
          <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            ...
            <Button
              ...
              isLoading={isSaving}
              onClick={() => {
                if (suspendTarget) void setStatus(suspendTarget, "SUSPENDED");
              }}
            >
              {t("suspendConfirm")}
            </Button>
