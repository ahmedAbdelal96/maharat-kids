...
export function isPaymentMethodAvailable(method: PaymentMethod): boolean {
  if (method.type === "CASH_ON_DELIVERY" || method.type === "MANUAL_TRANSFER") return true;
  return Boolean(method.enabled && method.providerKey && getPaymentGateway(method.providerKey));
}
...
export class PaymentMethodService {
  ...
  async getAvailable(): Promise<Result<PaymentMethod[], AppError>> { try { return success((await this.repository.findEnabled()).filter(isPaymentMethodAvailable)); } catch (error) { return failure(new AppError("PAYMENT_METHOD_OPERATION_FAILED", "Payment methods are unavailable.", { cause: error })); } }
