...
export async function uploadBankTransferProof(input: FormData) {
  ...
  const key = `payment-proofs/${randomUUID()}${extension}`; const root = path.join(process.cwd(), ".private"); await mkdir(path.join(root, "payment-proofs"), { recursive: true }); await writeFile(path.join(root, key), bytes, { flag: "wx" });
