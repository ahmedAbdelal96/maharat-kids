    ...
    <div className="flex min-h-[70vh] items-center justify-center px-4 py-12 sm:px-6">
      <Card className="w-full max-w-md p-6 sm:p-8">
        <CardHeader className="space-y-2 p-0 pb-6 text-center">
          ...
          <CardDescription>We sent a 6-digit code to <span className="font-semibold text-[var(--text-primary)]">{email}</span></CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          ...
          <form onSubmit={handleVerify} className="space-y-4">
            ...
            <Button type="submit" size="lg" isLoading={isLoading} className="w-full text-xs">Verify Code</Button>
          </form>
          <button type="button" onClick={handleResend} disabled={isResending} className="mx-auto mt-5 flex items-center gap-1.5 text-xs font-semibold text-[var(--primary)] hover:underline disabled:opacity-60"><RotateCcw className="h-3.5 w-3.5" />Resend code</button>
        </CardContent>
      </Card>
    </div>
  );
