...
export class PrismaProductRepository implements ProductRepository {
  ...
  private include = { category: { select: { name: true, slug: true } }, images: { ...imageInclude, include: { media: { select: { url: true } } } } } as const;
  private async withApprovedRatings(records: ProductRecord[]): Promise<Product[]> {
  ...
  async findById(id: ProductId) { const record = await this.db.product.findUnique({ where: { id }, include: this.include }); return record ? toProduct(record as ProductRecord) : null; }
  async findByIds(ids: ProductId[]) { if (ids.length === 0) return []; const records = await this.db.product.findMany({ where: { id: { in: ids } }, include: this.include }); return this.withApprovedRatings(records as ProductRecord[]); }
  async findBySlug(slug: string, publicOnly = false) { const record = await this.db.product.findFirst({ where: { slug, ...(publicOnly ? { status: "ACTIVE" } : {}) }, include: this.include }); return record ? toProduct(record as ProductRecord) : null; }
  ...
  async findPublic(input: ProductQuery): Promise<ProductPage> { const page = input.page ?? 1; const pageSize = input.pageSize ?? 24; const and: Prisma.ProductWhereInput[] = [{ status: "ACTIVE" }]; if (input.featured !== undefined) and.push({ isFeatured: input.featured }); if (input.search) and.push({ OR: [{ name: { contains: input.search, mode: "insensitive" } }, { sku: { contains: input.search, mode: "insensitive" } }, { shortDescription: { contains: input.search, mode: "insensitive" } }] }); if (input.categorySlug) and.push({ category: { slug: input.categorySlug, isActive: true } }); if (input.minPrice || input.maxPrice) and.push({ price: { ...(input.minPrice ? { gte: new Prisma.Decimal(input.minPrice) } : {}), ...(input.maxPrice ? { lte: new Prisma.Decimal(input.maxPrice) } : {}) } }); if (input.inStock) and.push({ OR: [{ trackInventory: false }, { trackInventory: true, stockQuantity: { gt: 0 } }] }); const where: Prisma.ProductWhereInput = { AND: and }; const [total, records] = await Promise.all([this.db.product.count({ where }), this.db.product.findMany({ where, include: this.include, orderBy: [{ isFeatured: "desc" }, { createdAt: "desc" }], skip: (page - 1) * pageSize, take: pageSize })]); return { items: await this.withApprovedRatings(records as ProductRecord[]), total, page, pageSize, totalPages: Math.max(1, Math.ceil(total / pageSize)) }; }
  async findAdmin(input: ProductQuery = {}): Promise<ProductPage> { const page = input.page ?? 1; const pageSize = input.pageSize ?? 48; const where: Prisma.ProductWhereInput = { ...(input.search ? { OR: [{ name: { contains: input.search, mode: "insensitive" } }, { sku: { contains: input.search, mode: "insensitive" } }] } : {}), ...(input.status ? { status: input.status } : {}), ...(input.categoryId ? { categoryId: input.categoryId } : {}) }; const [total, records] = await Promise.all([this.db.product.count({ where }), this.db.product.findMany({ where, include: this.include, orderBy: { updatedAt: "desc" }, skip: (page - 1) * pageSize, take: pageSize })]); return { items: records.map((record) => toProduct(record as ProductRecord)), total, page, pageSize, totalPages: Math.max(1, Math.ceil(total / pageSize)) }; }
