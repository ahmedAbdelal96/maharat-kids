import { z } from "zod";
export const placeOrderSchema = z.object({ addressId: z.string().trim().min(1).nullable().optional(), paymentMethodId: z.string().trim().min(1), checkoutToken: z.string().trim().min(16).max(100), paymentReference: z.string().trim().max(120).optional(), paymentNotes: z.string().trim().max(500).optional() });
