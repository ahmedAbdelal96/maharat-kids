...
export class PrismaDashboardRepository implements DashboardRepository {
  ...
  async getOverview(currency: string) {
    ...
    const [
      ...
      this.db.order.count({ where: { status: "PENDING" } }),
      this.db.order.count({ where: { paymentStatus: { in: ["PENDING", "PENDING_VERIFICATION"] }, paymentMethod: { is: { type: { in: ["MANUAL_TRANSFER", "BANK_TRANSFER"] } } } } }),
      this.db.order.count({ where: { paymentStatus: "PENDING_VERIFICATION", paymentMethod: { is: { type: { in: ["MANUAL_TRANSFER", "BANK_TRANSFER"] } } } } }),
