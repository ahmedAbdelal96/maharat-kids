  ...
  if (cart.items.length === 0) {
    return (
      <motion.div initial={reduceMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface-card)] px-6 py-16 text-center">
        ...
        <p className="mx-auto mt-2 max-w-md text-sm text-[var(--text-secondary)]">{t("browseToStart")}</p>
        <Link href="/products"><Button className="mt-6 gap-2">{t("continueShopping")} <ArrowRight className="h-4 w-4" /></Button></Link>
      </motion.div>
    );
  }
        ...
        <div className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-card)] p-4">
          {cart.coupon ? (
            <div className="flex flex-wrap items-center gap-3 text-sm">
              ...
              <div className="min-w-0 flex-1"><p className="font-bold">{cart.coupon.code}</p><p className="text-xs text-[var(--text-secondary)]">{Number(cart.couponDiscount) > 0 ? `${cart.coupon.name} ${t("applied")}.` : cart.couponMessage ?? t("couponSelected")}</p></div>
              {Number(cart.couponDiscount) > 0 && <Badge variant="success" size="sm">{t("save")} {formatMoney(cart.couponDiscount, currency)}</Badge>}
              <Button type="button" variant="ghost" size="sm" disabled={couponBusy} onClick={removeCoupon}>{actions("delete")}</Button>
            </div>
          ) : (
            <form className="flex flex-col gap-2 sm:flex-row" onSubmit={(event) => { event.preventDefault(); void applyCoupon(); }}>
              <label htmlFor="cart-coupon" className="sr-only">{t("couponCode")}</label>
              <input id="cart-coupon" value={couponCode} onChange={(event) => setCouponCode(event.target.value)} placeholder={t("haveCoupon")} className="h-10 min-w-0 flex-1 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] px-3 text-sm uppercase outline-none focus:border-[var(--border-focus)] focus:ring-2 focus:ring-[var(--ring)]" />
              <Button type="submit" size="sm" isLoading={couponBusy} disabled={!couponCode.trim()}>{actions("apply")}</Button>
            </form>
          )}
        </div>
              ...
              <motion.div key={`gift-${gift.productId}`} layout={!reduceMotion} initial={reduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-4 border-t border-[var(--border-subtle)] bg-[var(--surface-muted)]/30 p-5 sm:flex-row sm:items-center">
                <div className="flex min-w-0 flex-1 items-center gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-[var(--text-secondary)]">{t("freeGiftWith", { name: gift.promotionName })}</p>
                    <span className="mt-1.5 inline-flex items-center gap-1 text-[11px] font-bold text-[var(--success)]">
                      <Gift className="h-3.5 w-3.5" /> {t("free")} ({t("was")} {formatMoney(gift.originalPrice, currency)})
                    </span>
                  </div>
                </div>
                <div className="flex items-center justify-end gap-5">
                  <span className="text-xs font-semibold text-[var(--text-muted)]">{t("quantity", { count: gift.quantity })}</span>
                  <span className="min-w-24 text-right text-sm font-extrabold text-[var(--success)]">{t("free")}</span>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
        <Link href="/products" className="inline-flex items-center gap-1 text-xs font-semibold text-[var(--primary)]">← {t("continueShopping")}</Link>
      </div>
      <div className="rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface-card)] p-6">
        <h2 className="border-b border-[var(--border)] pb-3 text-lg font-bold">{t("orderSummary")}</h2>
          ...
          {Number(cart.discountAmount) > 0 && (
            <div className="flex justify-between text-xs text-[var(--primary)] font-semibold">
              <span className="flex items-center gap-1.5 truncate max-w-[200px]">
                ...
                {cart.appliedPromotion?.promotionName || t("promotionalOffer")}
              </span>
        ...
        {Number(cart.discountAmount) > 0 && (
          <p className="mt-3 rounded-[var(--radius-md)] bg-[var(--primary-soft)]/50 p-2 text-center text-xs font-semibold text-[var(--primary)]">
            {t("youSave", { amount: formatMoney(cart.discountAmount, currency) })}
          </p>
        )}
        <p className="mt-3 text-xs text-[var(--text-muted)]">{t("shippingPreparing")}</p>
        <Link href="/checkout" className="mt-6 block">
          <Button className="w-full gap-2">{t("proceedCheckout")} <ArrowRight className="h-4 w-4" /></Button>
        </Link>
      </div>
    </div>
  );
