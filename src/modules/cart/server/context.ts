...
export async function resolveShoppingCartContext(options: { createGuest?: boolean } = {}): Promise<Result<CartContext | null, AppError>> {
  ...
  if (current.data?.user.type === "CUSTOMER") return success({ kind: "customer", customerId: current.data.user.id });
  ...
  return success(token ? { kind: "guest", guestTokenHash: hashGuestToken(token) } : null);
}
...
export async function mergeGuestCartForCustomer(customerId: string): Promise<Result<CartMergeResult | null, AppError>> {
  ...
  const result = await service().mergeGuestCart(customerId, hashGuestToken(token));
