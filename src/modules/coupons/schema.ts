...
export const createCouponSchema = couponFields.superRefine((input, ctx) => {
  ...
  if (input.type === "FIXED_AMOUNT" && (!(input.fixedDiscountAmount && input.fixedDiscountAmount > 0))) ctx.addIssue({ code: "custom", path: ["fixedDiscountAmount"], message: "Fixed discount must be greater than 0." });
  if (input.type === "FIXED_AMOUNT" && input.maximumDiscountAmount != null) ctx.addIssue({ code: "custom", path: ["maximumDiscountAmount"], message: "Maximum discount applies only to percentage coupons." });
});
